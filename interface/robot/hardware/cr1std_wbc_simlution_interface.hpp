...
#ifndef CR1_STD_SIMULATION_HPP_
#define CR1_STD_SIMULATION_HPP_
...
namespace interface {
    class CR1_STD_WBC_SimInterface : public RobotInterface {
    private:
        double run_time_ = 0;
    ...
    public:
        CR1_STD_WBC_SimInterface(const std::string &name, int dof_num = 21) : RobotInterface(name, dof_num) {
            // run_cnt_ = 0;
            ...
            std::cout << name << " is using mujoco Simulation" << std::endl;
        }
        ...
        virtual void SetJointCommand(Eigen::Matrix<float, Eigen::Dynamic, 5> input) {
            // std::cout<<"SetJointCommand called with input: " << input << std::endl;
            joint_cmd_ = input;
        }

        virtual void Start() {
            start_thread_flag_ = true;
            sim_thread_ = std::thread(std::bind(&CR1_STD_WBC_SimInterface::ReceiveRobotData, this));
            send_thread_ = std::thread(std::bind(&CR1_STD_WBC_SimInterface::SendJointCmd, this));
