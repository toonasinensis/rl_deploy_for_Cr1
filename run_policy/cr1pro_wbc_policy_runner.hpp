...
private:
    VecXf kp_, kd_;
    VecXf dof_pos_default_policy; //dof_pos
    ...
    std::vector<std::string> policy_and_pd_order;
...
public:
    ...
    memory_info_(Ort::MemoryInfo::CreateCpu(OrtArenaAllocator, OrtMemTypeDefault))
    {
        ...
        for (int i = 0; i < dof_dim; ++i){
            ...
            std::cout << "policyandpd2robot_idx[" << i << "]: " << policyandpd2robot_idx[i] << std::endl;
        }
    ...
    RobotAction GetRobotAction(const RobotBasicState& ro, const UserCommand& uc){
        ...
        VecXf joint_pos_default = dof_pos_default_policy * obs_scales_joint_pos;// in rl squenece

        // std::cout<<"data_cnt"<<data_cnt<<"joint_data.size()"<<joint_data.size()<<std::endl;
        if( data_cnt >= joint_data.size()){data_cnt = 0;}//保持最后一帧数据
        ...
        for (int i =0;i<act_dim_;i++){
            ...
            // joint_pos_rl(i) = ro.joint_pos(i)*obs_scales_joint_pos;
        ...
        // std::cout << joint_pos_default << "default dof pos" << std::endl;
        ...
        for (int i = 0; i < obs_buff.size(); ++i) {
            ...
            if ((i + 1) % 10 == 0 || i == obs_buff.size() - 1) std::cout << std::endl;
        }
        ...
        // }
        // std::cout << std::endl;
// self._kp_scale * self.p_gains*(actions_scaled + self.default_dof_pos - self.simulator.dof_pos) - \
//             self._kd_scale * self.d_gains*self.simulator.dof_vel
        action_ += dof_pos_default_policy;
        ...
        // std::cout<<"dof_pos_default_policy"<<dof_pos_default_policy.transpose()<<std::endl;
    ...
    void SetDefaultJointPos(const VecXf& pos){
        dof_pos_default_policy.setZero(dof_dim); 
        for(int i=0;i<dof_dim;++i) {
            dof_pos_default_policy(i) = pos(i);
        }
    }
};
