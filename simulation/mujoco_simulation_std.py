...
MODEL_NAME = "CR01B-std"
XML_PATH = "urdf_model/CR01B-standard/B-20250522.xml"
LOCAL_PORT = 20001
CTRL_IP = "127.0.0.1"
CTRL_PORT = 30010
USE_VIEWER = True
...
URDF_INIT = {
    ...
    "CR01B-std": np.array([0, ] * 21, dtype=np.float32)
}
...
q_offset = np.array([0, ] * 21, dtype=np.float32)

q_offset[2] = -0.0 / 57.3   #主动加10度offset到waist y上
...
class MuJoCoSimulation:
    ...
    def _udp_receiver(self):
        ...
        while True:
            ...
            print(f"[UDP] Received {len(unpacked)} values from {addr}") 
            ...
            if self.virtual_joint is not None:
                ...
                self.pos_cmd = np.asarray(unpacked[self.dof_num:self.dof_num * 2], dtype=np.float32).reshape(self.dof_num, 1)
                self.kd_cmd = np.asarray(unpacked[self.dof_num * 2:self.dof_num * 3], dtype=np.float32).reshape(self.dof_num, 1)
                self.vel_cmd = np.asarray(unpacked[self.dof_num * 3:self.dof_num * 4], dtype=np.float32).reshape(self.dof_num, 1)
    ...
    def _apply_joint_torque(self):
        # 当前关节状态#TODO:
        q = self.data.qpos[ dof_Free_pos:dof_Free_pos + self.dof_num].reshape(-1, 1)
        dq = self.data.qvel[ dof_Free_vel:dof_Free_vel + self.dof_num].reshape(-1, 1)
        # import ipdb;ipdb.set_trace();
        q  = q.copy()
    ...
    def _set_mimic_init_state(self):
        ...
        self.data.qvel[:] = np.zeros((self.dof_num + 6, ), dtype=np.float64)
    ...
    def _send_robot_state(self):
        ...
        # ----- 关节 -----
        q = self.data.qpos[dof_Free_pos:dof_Free_pos+ self.dof_num]
        dq = self.data.qvel[dof_Free_vel : dof_Free_vel+ self.dof_num]
