...
private:
    ...
    void AddSlopeTerrain(){
        ...
        std::vector<double> height = {0, slope_tan, 2*slope_tan, slope_tan, 0, 
                                        ...
                                        0, slope_tan, 2*slope_tan, slope_tan, 0,
                                        0, slope_tan, 2*slope_tan, slope_tan, 0};
        auto heightMap = world_.addHeightMap(5, 5, 10, 10, 6, 0, height);//mat.rows, mat.cols, length, whidth, x, y, array
    }

    void AddStairTerrain(){
        ...
        auto plat = world_.addBox(plat_length, stair_length, stair_height*(stair_num+1), 1.e7);
    ...
    void AddIndustrialStairTerrain(){
        double stair_height = 0.18, stair_depth = 0.3, stair_length = 4.0;
        double init_x = 2.;
        int stair_num = 6;
        double plat_length = 1.5;
        const double box_len = 0.1;
        ...
        for(int i=0;i<stair_num;++i){
            ...
            std::string num_str = std::to_string(1. - i/double(stair_num));
            auto left_box = world_.addBox(stair_depth, box_len, box_height-board_thickness, 1.e7);
            left_box->setPosition(init_x+(i+0.5)*stair_depth , stair_length/2.-box_len/2., (box_height-board_thickness)/ 2.);
            left_box->setAppearance("0,"+ num_str + ",0,1.");

            auto right_box = world_.addBox(stair_depth, box_len, box_height-board_thickness, 1.e7);
            right_box->setPosition(init_x+(i+0.5)*stair_depth , -stair_length/2.+box_len/2., (box_height-board_thickness)/ 2.);
            right_box->setAppearance("0,"+ num_str + ",0,1.");

            auto mid_box = world_.addBox(stair_depth, stair_length, board_thickness, 1.e7);
            mid_box->setPosition(init_x+(i+0.5)*stair_depth , 0, box_height-board_thickness/ 2.);
            mid_box->setAppearance("0,"+ num_str + ",0,1.");
        }

        auto left_plat_box = world_.addBox(plat_length, box_len, stair_height*(stair_num+1)-board_thickness, 1.e7);
        left_plat_box->setPosition(init_x+stair_num*stair_depth+0.5*plat_length, stair_length/2.-box_len/2., (stair_height*(stair_num+1)-board_thickness)/ 2.);
        left_plat_box->setAppearance("yellow");

        auto right_plat_box = world_.addBox(plat_length, box_len, stair_height*(stair_num+1)-board_thickness, 1.e7);
        right_plat_box->setPosition(init_x+stair_num*stair_depth+0.5*plat_length, -stair_length/2.+box_len/2., (stair_height*(stair_num+1)-board_thickness)/ 2.);
        right_plat_box->setAppearance("yellow");

        auto mid_plat_box = world_.addBox(plat_length, stair_length, board_thickness, 1.e7);
        mid_plat_box->setPosition(init_x+stair_num*stair_depth+0.5*plat_length, 0, stair_height*(stair_num+1)-board_thickness/ 2.);
        mid_plat_box->setAppearance("yellow");

        for(int i=0;i<stair_num;++i){
            double box_height = stair_height*(stair_num - i);
            std::string num_str = std::to_string(1. - i/double(stair_num));
            auto left_box = world_.addBox(stair_depth, box_len, box_height-board_thickness, 1.e7);
            left_box->setPosition(init_x+stair_num*stair_depth+plat_length+(i+0.5)*stair_depth, stair_length/2.-box_len/2., (box_height-board_thickness)/ 2.);
            left_box->setAppearance(num_str+",0"+",0,1.");

            auto right_box = world_.addBox(stair_depth, box_len, box_height-board_thickness, 1.e7);
            right_box->setPosition(init_x+stair_num*stair_depth+plat_length+(i+0.5)*stair_depth, -stair_length/2.+box_len/2., (box_height-board_thickness)/ 2.);
            right_box->setAppearance(num_str+",0"+",0,1.");

            auto mid_box = world_.addBox(stair_depth, stair_length, board_thickness, 1.e7);
            mid_box->setPosition(init_x+stair_num*stair_depth+plat_length+(i+0.5)*stair_depth, 0, box_height-board_thickness/ 2.);
            mid_box->setAppearance(num_str+",0"+",0,1.");
        }
    }

    Vec3f omega_body_, rpy_, acc_;
    VecXf joint_pos_, joint_vel_, joint_tau_;
    VecXd init_joint_pos_;
    
    std::thread sim_thread_;

public:
    RaisimInterface(const std::string& robot_name, int dof_num=12):RobotInterface(robot_name, dof_num), 
    gyro_nd_(0, 0.0001), rpy_nd_(0, 0.005), acc_nd_(0, 0.0){
        dt_ = 0.001;
        run_cnt_ = 0;
        gravity_ = gravity;
        ...
        activation_key_ = "";
        urdf_file_ = "";
        std::cout << robot_name << " is using Raisim Simulation" << std::endl;
    }

    ~RaisimInterface(){}

    void SetInitJointPos(const VecXd& init_joint_pos){
        if(init_joint_pos.size() != dof_num_+7){
            std::cerr << "error init joint pos config" << std::endl;
        }
    ...
    void Run(){
        ...
        jueying_->setGeneralizedCoordinate(init_joint_pos_);
        AddStairTerrain();
        // AddSlopeTerrain();
        // auto box = world_.addBox(0.3, 4, 0.3, 1.e8);
        // box->setPosition(2. , 0, 0.15);
